...
describe('pwa-input component test', () => {
    ...
    xtest('when entering a value in the input, the event pwa-input-keyup should be fired', async () => {
        const inputElement = element.shadowRoot.querySelector('input');
        const spy = jest.fn();
    ...
    xtest('when doing blur in the input, the event pwa-input-change should be fired', async () => {
        const inputElement = element.shadowRoot.querySelector('input');
        const spy = jest.fn();
        element.addEventListener('pwa-input-keyup', spy);
